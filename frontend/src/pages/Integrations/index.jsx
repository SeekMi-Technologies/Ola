...
import { Button, Input, Switch, Row, Col, Tag, message, Tooltip } from 'antd';
...
const INITIAL_INTEGRATIONS = [
  {
    ...
    connected: true,
    popular: true,
    description: 'Sync customer conversion data and audience lists directly to Google Ads.',
    logo: (
  ...
  },
  {
    ...
    connected: true,
    popular: true,
    description: 'Create issues and manage software development workflows seamlessly.',
    logo: (
  ...
  },
  {
    ...
    connected: false,
    popular: true,
    description: 'Sync wiki pages, notes, and CRM databases directly with Notion.',
    logo: (
      <svg viewBox="0 0 24 24" width="22" height="22" fill="#000000">
  ...
  },
  {
    ...
    connected: false,
    popular: false,
    description: 'Prospect new leads and trigger sales engagement workflows.',
    logo: (
      <svg viewBox="0 0 24 24" width="22" height="22">
  ...
  },
  {
    ...
    connected: false,
    popular: false,
    description: 'Report frontend errors and backend server exceptions in real time.',
    logo: (
  ...
  },
  {
    ...
    connected: false,
    popular: true,
    description: 'Deploy branch previews, serverless APIs, and frontend client builds.',
    logo: (
      <svg viewBox="0 0 24 24" width="22" height="22" fill="#000000">
  ...
  },
  {
    ...
    connected: false,
    popular: true,
    description: 'Connect your WhatsApp Business account to send automated updates and chat with customers.',
    logo: (
      <svg viewBox="0 0 24 24" width="22" height="22">
...
export default function IntegrationsPage() {
  ...
  const handleToggleConnection = (id, name, currentStatus) => {
    ...
    if (currentStatus) {
      message.info(`${name} disconnected successfully.`);
    } else {
  ...
  const filteredIntegrations = useMemo(() => {
    return integrations.filter((item) => {
      ...
      // 3. Connected Switch filter
      const matchesConnected = showConnectedOnly ? item.connected : true;

      return matchesSearch && matchesTab && matchesConnected;
    });
  }, [integrations, searchQuery, activeTab, showConnectedOnly]);
    ...
    >
      ...
      <PageHeader
        ...
        extra={[
          ...
          <Button
            ...
            onClick={() => message.info('Custom MCP integration server configurations coming soon!')}
          >
            Add Custom MCP
          </Button>
        ]}
      ...
      >
        <p style={{ color: '#595959', fontSize: '15px', margin: '0 0 16px 0', fontFamily: 'Inter, system-ui, sans-serif' }}>
          Connect the tools you use and let OLA perform tasks across various apps.
        </p>
      </PageHeader>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          <button
            ...
            style={{
              border: 'none',
              background: activeTab === 'all' ? '#eaeaea' : 'transparent',
              color: activeTab === 'all' ? '#1f1f1f' : '#8c8c8c',
          ...
          <button
            ...
            style={{
              border: 'none',
              background: activeTab === 'popular' ? '#eaeaea' : 'transparent',
              color: activeTab === 'popular' ? '#1f1f1f' : '#8c8c8c',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <span style={{ color: '#8c8c8c', fontSize: '14px', fontFamily: 'Inter, sans-serif' }}>Show connected only</span>
          <Switch
      ...
      <Row gutter={[24, 24]}>
        {filteredIntegrations.map((item) => (
          <Col xs={24} sm={12} md={8} key={item.id}>
            <Tooltip title={item.description} placement="top" mouseEnterDelay={0.4}>
              <div
                className="whiteBox shadow"
                onClick={() => handleToggleConnection(item.id, item.name, item.connected)}
                style={{
                  display: 'flex',
                  alignItems: 'center',
                  ...
                  padding: '16px 20px',
                  height: '76px',
                  border: item.connected ? '1.5px solid #d4f4e2' : '1px solid #eeeeee',
                  background: '#ffffff',
                  cursor: 'pointer',
                  borderRadius: '12px',
                  transition: 'all 0.3s ease-in-out',
                  boxShadow: item.connected ? '0 2px 8px rgba(82, 196, 26, 0.05)' : undefined
                }}
              >
                {/* Logo and Name */}
