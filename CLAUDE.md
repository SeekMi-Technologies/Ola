...
- Config via `nanobot.config.json`: `channels.transcriptionProvider` (top-level under channels), `channels.transcriptionLanguage`; API key resolved from `providers.<provider>.apiKey` (e.g. `providers.openai.apiKey`). The Pydantic schema also accepts snake_case aliases, but saved JSON and deployment templates use camelCase.
...
OlaCrm — Ola Technologies 自有 MERN product，CRM/ERP for clients, quotes, invoices, purchase orders, payments, comparisons, reporting. Node.js 20 + Express + MongoDB backend, React 18 + Vite + Ant Design + Redux Toolkit frontend. Production target: `https://app.olatech.ai` (primary). 过渡期 `https://app.olajob.cn` 并存，T+2 周后下架。
